...
후속 사용자 결정으로 **개수 보존·고정 좌표·강제 재배치를 폐기**했다. MapKit Recipe 7이
기존 37개 식생 영역의 폴리곤·제외 영역·seed·3m 간격·700/800‰ 밀도를 읽어 생성한다.
`Zone.tree`는 `city-tree`, 수관 반폭 58cm, 여유 5cm를 지정한다. 전체 수관이 해당 영역
안에 들어가고 장애물과 간섭하지 않는 후보만 생성하며, 좁거나 막힌 영역은 0개일 수 있다.
현재 정의에서는 연습장 **27개**가 생성되고 도시의 저작된 가로수 **237개**는 유지된다.
27은 검사 결과이며 고정 목표가 아니다. 영역·밀도·간격을 편집하면 생성 결과도 바뀐다.
옛 `driving_school_vegetation_v2.json`은 보관 자료이며 활성 작성기는 읽지 않는다.
...
`compact_vegetation.py`는 같은 `city_assets.tree()`와 활성 식생 레시피 입력만 선언한다. 후보 생성·솎기·지면 고정·간섭 검사는 MapKit이 수행한다.
...
2026-09-12 식생 레시피 전환의 MapKit 432개 셀 생성/동일 패키지 재현과
실제 식생 셀 36개에서 27개 나무의 영역 포함·도로/건물 간격·고유 소유권·수관 비중첩을
확인했다. 현재 패키지 해시는 `d09c4ef73dfcddf43977424c2f5202783a3a526cda022df138fd48cd8d52e2ab`다.
Python 작성기 회귀 10개, 편집기 열기·Save As·재열기·동일 export 124개와
식생 모델/수관/여유 설정 저장·undo/redo를 포함한 저작 검사 98개를 통과했다.
이 집중 검증은 전체 코스 수동 주행/Android 실기기 수락을 대신하지 않는다.
