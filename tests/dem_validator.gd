...
func run() -> void:
	...
	panel.prepare()
	await wait_job()
	check(not panel.plan.is_empty() and not panel.get_ok_button().disabled,"local source size/projection review: "+ui.status_label.text+" | "+panel.summary.text)
	if panel.plan.is_empty():
		ui.queue_free();await process_frame;quit(1);return
	check(panel.plan.sources[0].source.sha256 == source_sha and panel.summary.text.contains("EGM2008") and panel.summary.text.contains("source accuracy"),"review source identity, vertical reference and accuracy")
	...
	var first_png := first_destination+".cell-0.png"
	var raster: Dictionary = panel.candidate.value.dem.sampling.duplicate(true)
	var result := {"review":receipt,"raster":raster,"outputs":[{"cell":reviewed_cell(panel.plan),"png_path":first_png,"png_sha256":FileAccess.get_sha256(first_png),"png_bytes":FileAccess.get_file_as_bytes(first_png).size()}]}
	var reviewed: Dictionary = panel.plan.duplicate(true)
	check(state()==before and FileAccess.get_sha256(first_destination+".source-0.tif")==source_sha,"capture and native staging preserve original document/source")
	...
	check(panel.candidate==null and state()==before and FileAccess.file_exists(first_destination+".source-0.tif"),"discard preserves completed COG and project")
	...
	bad=result.duplicate(true);bad.outputs[0].png_sha256="0".repeat(64)
...
func reviewed_cell(plan: Dictionary) -> Array:
	return plan.cells[0]
