...
static func validate(raw: Dictionary, boundary: Script) -> String:
	...
	for entry in meta.sources:
		if entry is not Dictionary or entry.size() != 5 or not source_ref(entry.get("way")) or entry.get("closed") is not bool or entry.get("refs") is not Array or entry.refs.size() < (4 if entry.closed else 2): return "Invalid OSM loop source."
	...
	for entry in meta.retained:
		...
		if refs is not Array or refs.size() != count or road.get("kind") != source.kind or road.get("clearance_cm") != source.clearance_cm or road.get("points") is not Array or road.points.size() != count: return "Invalid OSM loop geometry/profile."
