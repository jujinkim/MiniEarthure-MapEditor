...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='mapeditor-documents-') as directory, tempfile.TemporaryDirectory(prefix='run-', dir=data_root) as user_directory:
        ...
        if args.full:
            scripts += ['editor_validator', 'test_drive_validator', 'workbench_validator', 'authoring_validator', 'authoring_safety_validator', 'preview_export_validator', 'import_layer_validator', 'import_job_validator', 'projection_validator', 'heightmap_import_validator', 'osm_import_validator', 'download_validator', 'overture_validator', 'dem_validator']
        if args.script:
            scripts = args.script
        pack_args = []
